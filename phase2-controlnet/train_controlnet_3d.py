"""Train 3D ControlNet for mask-conditioned CT image generation.

Step 3 of the ControlNet pipeline:
  1. Image VAE (done) -> encodes CT to latent space
  2. Unconditional LDM (done) -> learns CT latent distribution
  3. ControlNet (this script) -> conditions on vessel mask

Following MONAI's 2d_controlnet tutorial pattern:
  - Load pretrained VAE (frozen) and DM (frozen)
  - Copy DM weights to ControlNet
  - Train only the ControlNet with mask conditioning
  - Use ControlNetLatentDiffusionInferer for latent-space training
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Train 3D ControlNet for mask→CT generation")
    # Data
    ...
    p.add_argument("--dm_checkpoint", type=str, required=True,
                    help="Path to trained unconditional DM weights")
    p.add_argument("--output_dir", type=str, default="./phase2-controlnet/outputs_controlnet")
    ...
    # UNet architecture (must match trained DM)
    ...
    p.add_argument("--cn_embedding_channels", type=int, nargs="+", default=[16],
                    help="ControlNet conditioning embedding intermediate channels. "
                         "Each additional entry adds a stride-2 downsampling step.")
    # Scheduler
...
def build_controlnet(args: argparse.Namespace) -> ControlNet:
    return ControlNet(
        spatial_dims=3,
        in_channels=args.latent_channels,
        num_channels=tuple(args.unet_num_channels),
        ...
        num_head_channels=tuple(args.unet_num_head_channels),
        with_conditioning=False,
        conditioning_embedding_in_channels=1,  # binary mask, 1 channel
...
def run_sampling(
    ...
    inferer = ControlNetLatentDiffusionInferer(ddim_scheduler, scale_factor=scale_factor)
    controlnet.eval()

    val_batch = next(iter(val_loader))
    masks = val_batch["mask"].to(device)
    gt_images = val_batch["image"].to(device)
    n = min(args.val_num_samples, masks.shape[0])

    noise = torch.randn((n,) + latent_shape, device=device)
    with torch.no_grad():
    ...
    for idx in range(n):
        save_controlnet_comparison(
            mask=masks[idx:idx+1].cpu(),
            generated=samples[idx:idx+1].cpu(),
            out_path=out_dir / f"epoch_{epoch+1:04d}_sample_{idx:02d}.png",
            gt_image=gt_images[idx:idx+1].cpu(),
        )
...
def main() -> None:
    ...
    train_ds = VesselImageMaskPtDataset(train_files, augment=args.augment, spatial_size=tuple(args.spatial_size))
    val_ds = VesselImageMaskPtDataset(val_files, augment=False, spatial_size=tuple(args.spatial_size))
    ...
    print(f"Loaded frozen VAE from {args.vae_checkpoint}")

    # 2. Load pretrained DM (frozen)
    unet = build_unet(args).to(device)
    dm_loaded = torch.load(args.dm_checkpoint, map_location="cpu")
    unet.load_state_dict(extract_state_dict(dm_loaded), strict=True)
    unet.eval()
    for param in unet.parameters():
        param.requires_grad = False
    print(f"Loaded frozen DM from {args.dm_checkpoint}")

    # 3. Create ControlNet and copy DM weights
    controlnet = build_controlnet(args).to(device)
    copy_result = controlnet.load_state_dict(unet.state_dict(), strict=False)
    print(f"Copied DM weights to ControlNet:")
    print(f"  Missing keys: {len(copy_result.missing_keys)}")
    print(f"  Unexpected keys: {len(copy_result.unexpected_keys)}")

    # Latent shape and scale factor
    sample_batch = next(iter(train_loader))["image"][:1].to(device)
    latent_shape = infer_latent_shape(autoencoder, sample_batch)

    # Try to load scale_factor from DM checkpoint
    dm_ckpt_full = dm_loaded if isinstance(dm_loaded, dict) else {}
    saved_sf = dm_ckpt_full.get("scale_factor", None) or dm_ckpt_full.get("args", {}).get("scale_factor", None)
    if saved_sf and float(saved_sf) > 0:
        scale_factor = float(saved_sf)
        print(f"Using scale_factor from DM checkpoint: {scale_factor:.6f}")
    ...
    inferer = ControlNetLatentDiffusionInferer(scheduler, scale_factor=scale_factor)

    # Optimizer (only ControlNet parameters)
    optimizer = torch.optim.Adam(controlnet.parameters(), lr=args.lr)
    ...
    print(
        f"Training ControlNet: {len(train_ds)} train, {len(val_ds)} val, {len(test_files)} test\n"
        f"  device={device}, amp={use_amp}, latent_shape={latent_shape}\n"
        f"  cn_embedding_channels={list(args.cn_embedding_channels)}"
    )
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(progress, start=1):
            ...
            if args.gradient_clip > 0:
                ...
                torch.nn.utils.clip_grad_norm_(controlnet.parameters(), args.gradient_clip)
        ...
        if (epoch + 1) % args.val_interval == 0 or epoch == 0 or (epoch + 1) == args.num_epochs:
            val_loss = validate_epoch(
                ...
                controlnet=controlnet, loader=val_loader,
                latent_shape=latent_shape, scheduler=scheduler,
            ...
            run_sampling(
                ...
                controlnet=controlnet, scale_factor=scale_factor,
                latent_shape=latent_shape, val_loader=val_loader,
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(controlnet.state_dict(), ckpt_dir / "controlnet_best_weights.pt")
    ...
    torch.save(controlnet.state_dict(), ckpt_dir / "controlnet_final_weights.pt")
    print(f"Training completed. Best val_mse: {best_val_loss:.6f}")
    print(f"Final weights: {ckpt_dir / 'controlnet_final_weights.pt'}")
